...
## ⚠️ ChatPromptTemplate 轉義注意事項

### 問題說明

當使用 LangChain 的 `ChatPromptTemplate` 時，模板會自動偵測 `{variable_name}` 格式作為變數佔位符。如果 prompt 文字中包含**任何**大括號 `{` 或 `}`，即使是在 JSON 範例、說明文字或註解中，都必須進行轉義，否則會導致以下錯誤：

```
Input to ChatPromptTemplate is missing variables {'\n"主要趨勢1"'}. 
Expected: ['\n"主要趨勢1"', 'input', 'memory'] 
Received: ['input', 'memory']
```

### 轉義規則

#### 1. **外部 `.txt` 檔案中的 JSON 範例**
所有 JSON 範例中的大括號都必須**雙倍轉義**為 `{{` 和 `}}`：
...
#### 3. **Python f-string 中的轉義**
如果 prompt 直接寫在 Python f-string 中（如 `prompt_manager.py` 的內嵌 prompt），需要**四倍轉義**：
...
原因：f-string 會先解析一次（`{{{{` → `{{`），然後 ChatPromptTemplate 再解析一次（`{{` → `{` 字面值）

### 危險模式清單

以下模式**絕對不可出現**在任何 prompt 檔案中：
...
# 檢查雙倍大括號和單一大括號的數量
grep -c '{{' steep_step5_infer.txt  # 應該 > 0
grep -c '{' steep_step5_infer.txt   # 每個 {{ 會被計算兩次，所以應該是 {{數量 × 2
```

### 修復案例

#### 案例 1：steep_step5_infer.txt（2025-12-09）
**問題**：說明文字中包含單一大括號
```
6. **請務必使用正確的 JSON 格式，使用大括號 {} 表示物件**
輸出格式舉例：{..."<i>時間尺度": "短期（1-3年）"}
```
...
#### 案例 2：steep_step3_aggregate.txt
**問題**：JSON 範例多行格式 + 單倍轉義
```
{
  "主要趨勢1": {
...
1. **所有 JSON 範例寫成單行**，避免任何換行導致的 `{\n"` 模式
2. **文字檔中全部使用雙倍轉義** `{{` `}}`
3. **Python f-string 中使用四倍轉義** `{{{{` `}}}}`
4. **任何含大括號的說明文字都要轉義**，包括中文說明
5. **每次修改 prompt 後立即檢查**是否有未轉義的大括號
6. **使用 grep 工具自動檢測**潛在問題
