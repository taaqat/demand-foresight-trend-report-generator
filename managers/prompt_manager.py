...
class PromptManager:
    ...
    class STEEP:
        ...
        def step2_prompt(topic, ver):
            ...
            11. 代表事件請簡要說明即可，不需要講太多細節。以兩句話為限！

            <output schema>：
        ("{topic}-v{ver}":("趨勢標題": "AI驅動下的數據中心擴張加劇碳排放壓力",
            "代表事件"：["Google的溫室氣體排放量在過去五年內激增48%,主要由於支撐AI系統的數據中心擴張。",
        ...
        **請使用繁體中文回答**"
        ...
        with open(os.path.join(_prompt_dir, 'steep_step3_aggregate.txt'), 'r', encoding='utf-8') as f:
            step3_prompt = f.read()


        # *** step 4: map events (news) back to each trend
        with open(os.path.join(_prompt_dir, 'steep_step4_classify.txt'), 'r', encoding='utf-8') as f:
            step4_prompt = f.read()

        # *** step 5: generate detailed inference
        with open(os.path.join(_prompt_dir, 'steep_step5_infer.txt'), 'r', encoding='utf-8') as f:
            step5_prompt = f.read()

        # *** step 6: conclude the trend report
        # todo:: allow customized length of output text
        with open(os.path.join(_prompt_dir, 'steep_step6_conclude.txt'), 'r', encoding='utf-8') as f:
            step6_prompt = f.read()
...
4. 想像你是要解釋這個趨勢給一位門外漢聽，因此趨勢洞察的部份請用淺顯易懂的方式描述，不要使用過長的句子或是過於艱深的字彙。
    ...
    **請確實針對內容裡的雙引號前面加上反斜線\進行轉譯**
    範例
    {{ "主要趨勢1": {{ "標題": "", "<a>趨勢洞察": "", "<b>Hashtag關鍵詞": [] }},
    "主要趨勢2": {{ }}
    }}
    ```
    請將遺漏的"雙引號"確實補上,並針對內容裡的雙引號前面加上\\進行轉譯。
    **請務必使用正確的 JSON 格式，使用大括號 {{}} 表示物件，方括號 [] 表示陣列。**
    """
...
對每個趨勢各對應3-4個最具啟發性的[代表事件]，並簡要分析其意義，每則代表事件加分析至少要30字，不超過45字，在每則代表事件及分析後面幫我括號（代表事件從summary檔案來源的對應新聞）
...
11. [重要]請不要過度思考。我需要快狠準的答案，請你把主要時間花在分析，而不是判斷是否為相關新聞上。

**請確實針對內容裡的雙引號前面加上反斜線\進行轉義**

```
<範例>(主要趨勢請參考此範例)
{{{{ "主要趨勢1": {{{{ "標題": "趨勢標題", "<a>趨勢洞察": "趨勢的詳細分析", "<b>Hashtag關鍵詞": ["#關鍵詞1", "#關鍵詞2".....], "<c>代表事件": [{{{{ "事件": "代表事件1", "分析": "事件1的分析", "來源": "總整我輸入的檔案來源中的對應新聞。", "關聯度": "0-100%" }}}}] }}}}
}}}}

** 請務必使用正確的 JSON 格式，使用大括號表示物件，方括號 [] 表示陣列。 **
...
{additional}
    """
        ...
        def get_key_data_from_pdf(topic, trend_report_json):
            return f"""你是一個精明的產業分析顧問。我根據近期的新聞輿論資料，撰寫了關於「{topic}」主題的趨勢報告，不過目前缺乏研究或是調查報告的佐證。我需要你協助我從一些調查報告或是研究當中，找出能支持我的趨勢報告的「關鍵數據或案例」。

我撰寫的趨勢報告如下：
{trend_report_json}

該報告包含了數個趨勢。接下來我會輸入一篇「研究報告」或是「調查報告」的文本，請你針對每個趨勢，從我輸入的文本中找出支持該趨勢的關鍵數據或案例。注意，必須要是「支持」該趨勢的關鍵數據或案例，而非只是與該趨勢有「相關性」。所謂的支持，可能是該數據或案例暗示「該趨勢確實存在」，或是該數據或案例「可能會對該趨勢的發展造成正面或是負面的影響」。

輸出格式如下：
( "<第一個趨勢的趨勢標題>": (
     "關鍵數據": [<第一個關鍵數據(來源頁數)>, <第二個關鍵數據(來源頁數)>, <第二個關鍵數據(來源頁數)>, ...]
    ,"案例": [<第一個案例(來源頁數)>, <第二個案例(來源頁數)>, <第二個案例(來源頁數)>, ...]
    ),
    "<第二個趨勢的趨勢標題>": (
     "關鍵數據": [<第一個關鍵數據(來源頁數)>, <第二個關鍵數據(來源頁數)>, <第二個關鍵數據(來源頁數)>, ...]
    ,"案例": [<第一個案例(來源頁數)>, <第二個案例(來源頁數)>, <第二個案例(來源頁數)>, ...]
...
一些注意事項：
1. 如果真的沒有關鍵數據或案例支持某趨勢，請在該趨勢下面的關鍵數據或是案例欄位中，回傳一個 list: ["無資料"]。不要硬生成一些完全跟趨勢無關的內容。
2. 關鍵數據或案例必須 100% 來自於研究報告，請不要造假。
3. 請一次回傳完整的結果，不要只回傳一部分的趨勢資料。每個趨勢都要回傳（就算沒有關鍵數據與案例支持）。
4. 除了 JSON 格式的回應，不需要問我其他問題，也不要回傳其他額外文字。
5. 每個關鍵數據與案例不超過 20 字。
6. 每個關鍵數據與案例後方必須用括號標註來源頁數。
7. 請將我上述的輸出格式中的小括號，轉成 JSON 的大括號之後輸出給我。
8. 只要生成 JSON body 給我，不要給我其他文字。
9. 請使用繁體中文輸出。
...
9. 不要添加任何的註解區塊！！
10. 請精簡表達即可，總 token 數目幫我控制在 10000 以內。
'''