class PromptManager:
    ...
    class SELF_SELECT:
        ...
        step2_prompt = lambda title, ver: f"""先審視完全部原文（每月新聞），假設你是一位產業分析師、政策制定人員,只基於文中實際提到的內容做以下產出：
...
***** 這次推論只需要請你推論以下部份：<a>趨勢洞察, <b>Hashtag關鍵詞, <c>代表事件, {cols}。其他部份可以忽略不做。輸出格式請一樣用 well-formatted JSON *****
***** 其他額外指示：
{additional}。
麻煩你了！
'''
